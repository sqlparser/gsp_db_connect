...
public class JdbcUrlParser {
    ...
    public static TSQLDataSource generateSQLDataSource(EDbVendor vendor, Class<?> driver, String jdbcUrl,
        ...
        switch (vendor) {
            case dbvgreenplum:
                return new TGreenplumSQLDataSource(driver, jdbcUrl, account, password);
            case dbvmysql:
                return new TMysqlSQLDataSource(driver, jdbcUrl, account, password);
            case dbvhive:
                return new THiveMetadataDataSource(driver, jdbcUrl, account, password);
            case dbvmssql:
                return new TMssqlSQLDataSource(driver, jdbcUrl, account, password);
            case dbvazuresql:
                String database = null;
                if (jdbcUrl.indexOf("databasename=") != -1) {
                    database = jdbcUrl.substring(jdbcUrl.indexOf("databasename=") + "databasename=".length());
                    if (database.indexOf(";") != -1) {
                        database = database.substring(0, database.indexOf(";"));
                    }
                }
                return new TAzureSQLDataSource(driver, jdbcUrl, account, password, database);
            case dbvteradata:
                return new TTeradataSQLDataSource(driver, jdbcUrl, account, password);
            case dbvnetezza:
                return new TNetezzaSQLDataSource(driver, jdbcUrl, account, password);
            case dbvoracle:
                return new TOracleSQLDataSource(driver, jdbcUrl, account, password);
            case dbvpostgresql:
                return new TPostgreSQLDataSource(driver, jdbcUrl, account, password);
            case dbvredshift:
                return new TRedshiftSQLDataSource(driver, jdbcUrl, account, password);
            case dbvsnowflake:
                return new TSnowflakeSQLDataSource(driver, jdbcUrl, account, password);
            default:
                throw new UnsupportedOperationException(
                        "Unsupported the " + vendor.name().replace("dbv", "") + " jdbc url: " + jdbcUrl);
        }
